    ...
    renderDanhSachGhe = (danhSachGhe, hangGhe) => {
        ...
        return danhSachGhe.map((ghe, index) => {
            ...
            if (hangGhe !== '') {
                ...
                }} key={index} className={`ghe ${classGheDuocChon} ${classGheDangDat}`}>{ghe.soGhe}</button>
            }
...
//Viết hàm mapDispatch để gửi dữ liệu lên redux mỗi khi có sự kiện
const mapDispatchToProps = (dispatch) => {

    return {
        datGhe : (thongTinGhe) => {
            console.log('thongTinGhe',thongTinGhe);
            //Khai báo action
            const action = {
                type:'DAT_GHE',
