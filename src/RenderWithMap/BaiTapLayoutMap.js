...
export default class BaiTapLayoutMap extends Component {
    ...
    renderPhim = () => {
        const phimJSX = this.dataPhim.map((phim, index) => {
            return <div className='col-4 mt-5' key={index}>
                <div className="card text-left">
                    <div style={{ background: `url('${phim.hinhAnh}') center `, backgroundSize: 'cover' }}>
                        <img style={{ opacity: '0' }} className="card-img-top" src={phim.hinhAnh} alt />
                    </div>
                    <div className="card-body">
                        <h4 style={{ height: 50 }} className="card-title">{phim.tenPhim}</h4>
                        <p style={{ height: 70 }} className="card-text">
                            {phim.moTa.length > 100 ? <span>{phim.moTa.substr(0, 100)}...</span> : <span>{phim.moTa}</span>}</p>
                    </div>
                </div>
            </div>
